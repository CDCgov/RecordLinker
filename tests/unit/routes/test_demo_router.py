...
class TestResetDemo:
    def test_reset_demo(self, client):
        ...
        assert response.status_code == 200
        assert response.json() == {"message": "Demo data reset successfully."}

    def test_reset_demo_with_session_update(self, client):
        # Simulate a session store update
        patient_reference_id = 1
        response = client.post(f"/api/demo/record/{patient_reference_id}/unlink")
        ...
        # Make a request to check if the session is cleared
