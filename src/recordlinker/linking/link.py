    ...
    for algorithm_pass in algorithm.passes:
        with TRACER.start_as_current_span("link.pass"):
            ...
            with TRACER.start_as_current_span("link.block"):
                ...
                candidate_patients_query = mpi_service.get_block_data(session, record, algorithm_pass)
                # fetch all remaining Patient records within the Person clusters identified in
                # blocking for Belongingess Ratio calculation
                patients = mpi_service.fetch_person_records(session, candidate_patients_query)
